...
def init_db():
    c = con()
    c.execute('CREATE TABLE IF NOT EXISTS rooms(code TEXT PRIMARY KEY,current_round INTEGER NOT NULL DEFAULT 1,created_at REAL NOT NULL)')
    c.execute('CREATE TABLE IF NOT EXISTS teams(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT NOT NULL,name TEXT NOT NULL,UNIQUE(room,name))')
    c.execute('CREATE TABLE IF NOT EXISTS answers(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT NOT NULL,team TEXT NOT NULL,round_no INTEGER NOT NULL,question_no INTEGER NOT NULL,points INTEGER NOT NULL,detail TEXT,UNIQUE(room,team,round_no,question_no))')
    cols = [r['name'] for r in c.execute('PRAGMA table_info(answers)').fetchall()]
    if 'question_no' not in cols:
        c.execute('DROP TABLE answers')
        c.execute('CREATE TABLE answers(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT NOT NULL,team TEXT NOT NULL,round_no INTEGER NOT NULL,question_no INTEGER NOT NULL,points INTEGER NOT NULL,detail TEXT,UNIQUE(room,team,round_no,question_no))')
    c.commit(); c.close()
...
def calc_score(rnd,qno,answer):
    ...
    if q['type'] == 'text':
        ...
        return (q['points'] if any(norm(a) == norm(x) for x in q['answers']) else 0, a)
    if q['type'] == 'words':
        words=[]
        for w in re.split(r'[,;\n]+',str(answer or '')):
            w=norm(w)
...
async function setRound(r){await api('/api/set-round',{room,round:r});refresh()}async function resetAll(){if(confirm('Тазалансын ба?')){await api('/api/reset',{room});refresh()}}async function copyLink(){await navigator.clipboard.writeText(studentURL)}
async function refresh(){const s=await api('/api/state?room='+room+'&_='+Date.now());rt.textContent=s.round+' — '+titles[s.round];buttons.innerHTML=Object.keys(titles).map(x=>`<button class='${+x===s.round?'green':''}' onclick='setRound(${x})'>${x}. ${titles[x]}</button>`).join('');const a=[...s.teams].sort((x,y)=>y.score-x.score);tb.innerHTML=a.length?a.map((t,i)=>`<tr><td>${i==0?'🥇':i==1?'🥈':i==2?'🥉':i+1}</td><td><b>${t.name}</b></td><td>${t.score}</td><td>${t.done}</td></tr>`).join(''):'<tr><td colspan=4>Командалар әлі қосылған жоқ.</td></tr>'}refresh();setInterval(refresh,1200);
</script></body></html>"""

PLAY = """<!doctype html><html lang='kk'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><style>{{css}}</style></head><body><header><h1>🏆 ТІЛ БІЛГІРЛЕРІ LIVE</h1><div>Бөлме: {{room}}</div></header><div class='wrap'><div class='card' id='joinCard'><h2>Команда ретінде қосылу</h2><input id='teamName' placeholder='Команда атауы'><button onclick='joinTeam()'>Қосылу</button></div><div class='card hidden' id='gameCard'><span class='badge' id='badge'></span><h2 id='title'></h2><div id='count'></div><div class='progress'><div id='bar' class='bar'></div></div><div>Команда: <b id='teamLabel'></b></div><div class='score'>Жалпы ұпай: <span id='score'>0</span></div><hr><div id='task'></div><div id='result'></div></div></div><script>
const room={{room|tojson}},R={{rounds|tojson}};let team=localStorage.getItem('team_'+room)||'',currentRound=0,answered=[];
async function api(p,d){let o={};if(d)o={method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)};return await(await fetch(p,o)).json()}
...
function render(){const rd=R[currentRound],qs=rd.questions,next=qs.findIndex((_,i)=>!answered.includes(i));badge.textContent='Кезең '+currentRound;title.textContent=currentRound+'-кезең. '+rd.title;if(next===-1){count.textContent=qs.length+'/'+qs.length+' тапсырма орындалды';bar.style.width='100%';task.innerHTML='<p class="ok">✅ Бұл кезең толық аяқталды. Келесі кезеңді күтіңіз.</p>';return}const q=qs[next];count.textContent=(next+1)+'/'+qs.length+' сұрақ';bar.style.width=Math.round(next/qs.length*100)+'%';if(q.type==='mc'){task.innerHTML=`<p><b>${q.q}</b></p>`+q.options.map((o,j)=>`<div class='option' data-i='${j}' onclick='pick(this)'>${o}</div>`).join('')+`<button onclick='submitMC(${next})'>Жауапты жіберу</button>`}else{task.innerHTML=`<p><b>${q.q}</b></p><textarea id='ans' rows='${q.type==='essay'?5:3}'></textarea><button onclick='submitText(${next})'>Жауапты жіберу</button>`}}
async function sendAnswer(qno,answer){const x=await api('/api/submit',{room,team,round:currentRound,question:qno,answer});if(!x.ok){result.innerHTML='<p class="bad">'+(x.error||'Қате')+'</p>';return}result.innerHTML=`<p class='ok'>✅ Жауап қабылданды. +${x.points} ұпай</p>`;await refresh();setTimeout(()=>result.innerHTML='',600)}
...
@app.route('/join')
def join_redirect(): return redirect(url_for('play',room=request.args.get('room','').upper()))
@app.route('/play/<room>')
def play(room):
    room=room.upper(); c=con(); ok=c.execute('SELECT 1 FROM rooms WHERE code=?',(room,)).fetchone(); c.close()
    if not ok:return 'Бөлме табылмады',404
...
def team_state():
    ...
    if not rr:c.close();return jsonify(error='Бөлме жоқ'),404
    rnd=rr['current_round']; ans=[x['question_no'] for x in c.execute('SELECT question_no FROM answers WHERE room=? AND team=? AND round_no=? ORDER BY question_no',(room,team,rnd)).fetchall()]; sc=c.execute('SELECT COALESCE(SUM(points),0) s FROM answers WHERE room=? AND team=?',(room,team)).fetchone()['s']; c.close()
    return jsonify(round=rnd,answered=ans,score=sc)
@app.post('/api/join')
def join_api():
    d=request.get_json(force=True); room=str(d.get('room','')).upper(); team=str(d.get('team','')).strip()[:40]; c=con()
...
def set_round():
    d=request.get_json(force=True); c=con(); c.execute('UPDATE rooms SET current_round=? WHERE code=?',(int(d['round']),str(d['room']).upper())); c.commit(); c.close(); return jsonify(ok=True)
@app.post('/api/reset')
def reset_api():
    d=request.get_json(force=True); room=str(d['room']).upper(); c=con(); c.execute('DELETE FROM answers WHERE room=?',(room,)); c.execute('DELETE FROM teams WHERE room=?',(room,)); c.execute('UPDATE rooms SET current_round=1 WHERE code=?',(room,)); c.commit(); c.close(); return jsonify(ok=True)
@app.post('/api/submit')
def submit_api():
    ...
    pts,detail=calc_score(rnd,qno,d.get('answer')); c.execute('INSERT INTO answers(room,team,round_no,question_no,points,detail) VALUES(?,?,?,?,?,?)',(room,team,rnd,qno,pts,detail)); c.commit(); c.close(); return jsonify(ok=True,points=pts)
