...
class DBConn:
    def __init__(self):
        ...
        if self.pg:
            if psycopg is None:
                raise RuntimeError("PostgreSQL драйвері орнатылмаған")
...
let team=localStorage.getItem("team_"+room)||"",currentRound=0,answered=[],renderedKey="";
async function api(p,d){let o={};if(d)o={method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(d)};return await(await fetch(p,o)).json()}
...
function render(){
 ...
 const key=currentRound+":"+next;
 if(next===-1){
   count.textContent=qs.length+"/"+qs.length+" тапсырма орындалды";
   bar.style.width="100%";
   if(renderedKey!==key){task.innerHTML='<p class="ok">✅ Бұл кезең толық аяқталды. Келесі кезеңді күтіңіз.</p>';renderedKey=key}
   return;
 }
 const q=qs[next];
 count.textContent=(next+1)+"/"+qs.length+" сұрақ";
 bar.style.width=Math.round(next/qs.length*100)+"%";
 if(renderedKey===key) return;
 renderedKey=key;
 if(q.type==="mc"){
   task.innerHTML=`<p><b>${q.q}</b></p>`+q.options.map((o,j)=>`<div class="option" data-i="${j}" onclick="pick(this)">${o}</div>`).join("")+`<button onclick="submitMC(${next})">Жауапты жіберу</button>`;
 } else {
   task.innerHTML=`<p><b>${q.q}</b></p><textarea id="ans" rows="${q.type==='essay'?5:3}" inputmode="text" autocomplete="off" autocapitalize="sentences" placeholder="Жауабыңызды жазыңыз"></textarea><button onclick="submitText(${next})">Жауапты жіберу</button>`;
 }
}
...
def join_api():
    ...
    c.execute("INSERT INTO teams(room,name) VALUES(?,?) ON CONFLICT(room,name) DO NOTHING",(room,team));c.commit();c.close()
