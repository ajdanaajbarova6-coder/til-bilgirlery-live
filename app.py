...
def init():
    c=con()
    c.executescript('''
    CREATE TABLE IF NOT EXISTS rooms(code TEXT PRIMARY KEY,current_round INTEGER DEFAULT 1,created_at REAL);
    CREATE TABLE IF NOT EXISTS teams(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT,name TEXT,UNIQUE(room,name));
    CREATE TABLE IF NOT EXISTS answers(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT,team TEXT,round_no INTEGER,question_no INTEGER,points INTEGER,detail TEXT,UNIQUE(room,team,round_no,question_no));
    ''')
    cols=[x['name'] for x in c.execute('PRAGMA table_info(answers)')]
    if 'question_no' not in cols:
        c.execute('DROP TABLE answers')
        c.execute('CREATE TABLE answers(id INTEGER PRIMARY KEY AUTOINCREMENT,room TEXT,team TEXT,round_no INTEGER,question_no INTEGER,points INTEGER,detail TEXT,UNIQUE(room,team,round_no,question_no))')
    c.commit(); c.close()
init()
...
def make_room():
    ...
    while True:
        code=''.join(secrets.choice(chars) for _ in range(5)); c=con()
        try:
            c.execute('INSERT INTO rooms VALUES(?,?,?)',(code,1,time.time())); c.commit(); c.close(); return code
...
def score_answer(r,q,a):
    ...
    if x['type']=='text':
        a=str(a or '').strip(); return (x['points'] if any(norm(a)==norm(z) for z in x['answers']) else 0,a)
    if x['type']=='words':
        words=[]
        for z in re.split(r'[,;\n]+',str(a or '')):
            z=norm(z)
...
const room={{room|tojson}},titles={{titles|tojson}},studentUrl=location.origin+'/play/'+room;link.textContent=studentUrl;
...
async function setr(r){await api('/api/set-round',{room,round:r});refresh()} async function resetAll(){if(confirm('Тазалансын ба?')){await api('/api/reset',{room});refresh()}} async function copyLink(){await navigator.clipboard.writeText(studentUrl)}
async function refresh(){let s=await api('/api/state?room='+room);rt.textContent=s.round+' — '+titles[s.round];buttons.innerHTML=Object.keys(titles).map(x=>`<button class='${+x===s.round?'green':''}' onclick='setr(${x})'>${x}. ${titles[x]}</button>`).join('');let a=[...s.teams].sort((x,y)=>y.score-x.score);tb.innerHTML=a.length?a.map((t,i)=>`<tr><td>${i==0?'🥇':i==1?'🥈':i==2?'🥉':i+1}</td><td><b>${t.name}</b></td><td>${t.score}</td><td>${t.done}</td></tr>`).join(''):'<tr><td colspan=4>Командалар әлі қосылған жоқ.</td></tr>'}refresh();setInterval(refresh,1200);
</script></body></html>"""

PLAY="""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><style>{{css}}</style></head><body><header><h1>🏆 ТІЛ БІЛГІРЛЕРІ LIVE</h1><div>Бөлме: {{room}}</div></header><div class='wrap'><div class='card' id='join'><h2>Команда ретінде қосылу</h2><input id='teamName' placeholder='Команда атауы'><button onclick='joinTeam()'>Қосылу</button><div id='msg'></div></div><div class='card hidden' id='game'><span class='badge' id='badge'></span><h2 id='title'></h2><div id='count'></div><div class='progress'><div id='bar' class='bar'></div></div><div>Команда: <b id='teamLabel'></b></div><div class='score'>Жалпы ұпай: <span id='score'>0</span></div><hr><div id='task'></div><div id='result'></div></div></div><script>
const room={{room|tojson}},R={{rounds|tojson}};let team=localStorage.getItem('team_'+room)||'',cr=0,answered=[];
async function api(p,d){let o={};if(d)o={method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)};return await(await fetch(p,o)).json()}
async function joinTeam(){let n=teamName.value.trim();if(!n)return;let x=await api('/api/join',{room,team:n});if(x.ok){team=n;localStorage.setItem('team_'+room,team);show()}}
async function show(){join.classList.add('hidden');game.classList.remove('hidden');teamLabel.textContent=team;await api('/api/join',{room,team});refresh()}
function pick(e){document.querySelectorAll('.option').forEach(x=>x.classList.remove('sel'));e.classList.add('sel')}
function render(){let rd=R[cr],qs=rd.questions,i=qs.findIndex((_,j)=>!answered.includes(j));badge.textContent='Кезең '+cr;title.textContent=cr+'. '+rd.title;if(i<0){count.textContent=qs.length+'/'+qs.length+' тапсырма орындалды';bar.style.width='100%';task.innerHTML='<p class=ok>✅ Кезең аяқталды. Келесі кезеңді күтіңіз.</p>';return}let q=qs[i];count.textContent=(i+1)+'/'+qs.length+' сұрақ';bar.style.width=Math.round(i/qs.length*100)+'%';if(q.type==='mc'){task.innerHTML=`<p><b>${q.q}</b></p>`+q.options.map((o,j)=>`<div class='option' data-i='${j}' onclick='pick(this)'>${o}</div>`).join('')+`<button onclick='smc(${i})'>Жауапты жіберу</button>`}else{task.innerHTML=`<p><b>${q.q}</b></p><textarea id='ans' rows='${q.type==='essay'?6:3}'></textarea><button onclick='st(${i})'>Жауапты жіберу</button>`}}
async function send(q,a){let x=await api('/api/submit',{room,team,round:cr,question:q,answer:a});if(x.ok){result.innerHTML=`<p class=ok>✅ +${x.points} ұпай</p>`;answered.push(q);setTimeout(()=>{result.innerHTML='';render();refresh()},600)}else result.innerHTML='<p class=bad>'+x.error+'</p>'}
function smc(q){let e=document.querySelector('.option.sel');if(!e){result.innerHTML='<p class=bad>Жауапты таңдаңыз.</p>';return}send(q,+e.dataset.i)} function st(q){let v=ans.value.trim();if(!v){result.innerHTML='<p class=bad>Жауап жазыңыз.</p>';return}send(q,v)}
async function refresh(){if(!team)return;let s=await api('/api/team-state?room='+room+'&team='+encodeURIComponent(team));score.textContent=s.score||0;cr=s.round;answered=s.answered||[];render()}if(team)show();setInterval(refresh,1500);
</script></body></html>"""
...
def teamstate():
    ...
    if not rr:c.close();return jsonify(error='Бөлме жоқ'),404
    rnd=rr['current_round']; ans=[x['question_no'] for x in c.execute('SELECT question_no FROM answers WHERE room=? AND team=? AND round_no=?',(room,team,rnd))]
    sc=c.execute('SELECT COALESCE(SUM(points),0) s FROM answers WHERE room=? AND team=?',(room,team)).fetchone()['s']; c.close()
...
def submit():
    ...
    pts,detail=score_answer(rnd,q,d.get('answer')); c.execute('INSERT INTO answers(room,team,round_no,question_no,points,detail) VALUES(?,?,?,?,?,?)',(room,team,rnd,q,pts,detail)); c.commit(); c.close(); return jsonify(ok=True,points=pts)
